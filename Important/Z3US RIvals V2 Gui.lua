...
-- Library functions 
    ...
    -- Library element functions
                ...
                -- Multi Sections
                    ...
                    for _, section in cfg.tabs do
                        ...
                        local multi_items = data.items; do 
                            ...
                            -- Tab 
                                multi_items[ "tab" ] = library:create( "ScrollingFrame" , {  -- was Frame
                                    Parent = library.cache;
                                    BackgroundTransparency = 1;
                                    Name = "\0";
                                    BorderColor3 = rgb(0, 0, 0);
        ...
        function library:dropdown(options) 
            ...
            local function point_in_gui(gui, x, y)
            ...
            local function is_click_inside(x, y)
                return point_in_gui(items[ "dropdown_object" ], x, y) or point_in_gui(items[ "dropdown_holder" ], x, y)
            end
            ...
            items[ "dropdown_object" ].MouseButton1Click:Connect(toggle_dropdown)
            items[ "dropdown" ].MouseButton1Click:Connect(toggle_dropdown)

            library:connection(uis.InputBegan, function(input, gameProcessedEvent)
                if cfg.open and input.UserInputType == Enum.UserInputType.MouseButton1 and not gameProcessedEvent then
                    local x = input.Position.X
                    local y = input.Position.Y

                    if not is_click_inside(x, y) then
                        cfg.open = false
                        cfg.set_visible(false)
                        library:close_element()
                    end
