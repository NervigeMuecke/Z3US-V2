...
function Library:InitEsp(Data)
    ...
    do
        ...
        Objects["BoxFillGradient"] = self:CreateObjects("UIGradient", {
            ...
            Rotation = Table['Boxes']['Filled']['Rotation'],
            Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 0, 0)),
                ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 255, 255)),
            }),
...
function Library:AddTarget(Player)
    ...
    function GetEquippedWeapon(player)
        ...
        for _, model in viewModels:GetChildren() do
            if string.match(model.Name, "^" .. player.Name .. " -") then
                ...
                return parts[2] and parts[2]:gsub("^%s*(.-)%s*$", "%1") or "None"
            end
        end
        return "None"
    end
    ...
    local ChildHandler = {}; do
        function ChildHandler.BindChildren(Character)
            ...
            Data['CurrentTool'] = GetEquippedWeapon(Player)
        end
